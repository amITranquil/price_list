...
powershell -Command "Compress-Archive -Path 'build\windows\x64\runner\Release\*' -DestinationPath 'price_list_windows_v2.6.0.zip' -Force"
if errorlevel 1 (
...
echo - price_list_windows_v2.6.0.zip
echo - build\windows\x64\runner\Release\ klasörü
echo.
echo GitHub Release güncellemesi için:
echo gh release delete-asset v2.6.0 price_list_windows_v2.6.0.zip --yes
echo gh release upload v2.6.0 price_list_windows_v2.6.0.zip
echo.
pause
